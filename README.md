...
- 图谱交互增强：支持节点拖拽、画布平移、滚轮缩放、`Fit view` 视口重置，并在大图场景下自动启用更轻量的标签与布局保护策略。
- 路径分析：可为两个节点设置起点/终点，并高亮当前图中的最短可用路径。
- 子图展开与折叠：选中节点后可显式展开邻居或折叠已加载的子图，避免信息过载。
- 国际化支持：内置中文 / English 一键切换，优先读取本地语言偏好，其次根据浏览器语言自动选择默认语言。
- 结构化详情展示：右侧 `metadata` 面板使用表格展示，并提供字段筛选与排序入口，便于后续扩展更多字段能力。
